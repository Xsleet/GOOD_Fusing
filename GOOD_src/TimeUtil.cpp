...
{
    int hh_tmp = (int)std::floor(sod / 3600.0 + 1.0e-9);
    if (hh)
        *hh = hh_tmp;

    double tmp = sod - hh_tmp * 3600.0;
    int minu_tmp = (int)std::floor(tmp / 60.0 + 1.0e-9);
    if (minu)
        *minu = minu_tmp;
...
void TimeUtil::yrdoy2ymd(int year, int doy, int *month, int *day)
{
    ...
    if (month)
        *month = (int)floor(date[1]);
    if (day)
        *day = (int)floor(date[2]);
} /* end of yrdoy2ymd */
...
gtime_t TimeUtil::gpst2utc(gtime_t tt_gps)
{
    ...
    for (int i = 0; leaps[i][0] > 0; i++)
    {
        ...
        if (TimeDiff(tu, ymdhms2time(leaps[i])) >= 0.0)
            return tu;
    }
...
gtime_t TimeUtil::utc2gpst(gtime_t tt_utc)
{
    for (int i = 0; leaps[i][0] > 0; i++)
    {
        if (TimeDiff(tt_utc, ymdhms2time(leaps[i])) >= 0.0)
            return TimeAdd(tt_utc, -leaps[i][6]);
    }
...
bool TimeUtil::str2time(const char *s, int i, int n, gtime_t &tt)
{
    ...
    *p = '\0';
    ...
    if (sscanf(str, "%lf %lf %lf %lf %lf %lf", &date[0], &date[1], &date[2],
               &date[3], &date[4], &date[5]) < 6)
        return false;
    if (date[0] < 100.0)
        date[0] += date[0] < 80.0 ? 2000.0 : 1900.0;
    tt = ymdhms2time(date);
